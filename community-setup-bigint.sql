-- ================================================
-- COMMUNITY Q&A DATABASE SETUP (BIGINT VERSION)
-- Compatible with existing questions table using bigint IDs
...
-- Step 2: Question likes table (using bigint to match questions.id)
...
-- ================================================
-- DATABASE FUNCTIONS (BIGINT VERSION)
-- ================================================

-- Increment question likes
CREATE OR REPLACE FUNCTION increment_question_likes(question_id BIGINT)
RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION decrement_question_likes(question_id BIGINT)
RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION increment_question_votes(question_id BIGINT)
RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION decrement_question_votes(question_id BIGINT)
RETURNS void AS $$
BEGIN
  UPDATE questions SET vote_score = vote_score - 1 WHERE id = question_id;
END;
...
CREATE OR REPLACE FUNCTION increment_answer_votes(answer_id BIGINT)
RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION decrement_answer_votes(answer_id BIGINT)
RETURNS void AS $$
BEGIN
  UPDATE answers SET vote_score = vote_score - 1 WHERE id = answer_id;
END;
...
-- ================================================
-- Verify setup:
-- SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND (tablename LIKE '%question%' OR tablename LIKE '%answer%');
